...
export default {
  async fetch(request, env, ctx) {
    ...
    const blockedByPrefix = pLower.startsWith('/.sisyphus/') || pLower === '/.sisyphus' || pLower.startsWith('/.wrangler/') || pLower === '/token-usage-output.txt';
    if (_blockedPaths.has(url.pathname) || _blockedPaths.has(pLower) || blockedByPrefix) {
    ...
    if (url.pathname.startsWith('/api/')) {
      return handleApiRest(request, env, ctx, url);
    }
...
function getAdminSessionTokenFromRequest(request, parsedBody) {
  const authHeader = request.headers.get('Authorization') || '';
  const bearer = authHeader.startsWith('Bearer ') ? authHeader.slice(7).trim() : '';
  const headerToken = request.headers.get('X-Admin-Session-Token') || request.headers.get('x-admin-session-token') || '';
  const bodyToken = parsedBody && typeof parsedBody === 'object'
    ? (parsedBody.auth_session_token || parsedBody.admin_session_token || parsedBody.session_token || '')
    : '';
  return String(bearer || headerToken || bodyToken || '').trim();
}

async function parseRequestBodyObject(request) {
  try {
    const contentType = String(request.headers.get('Content-Type') || '').toLowerCase();
    if (request.method === 'GET' || request.method === 'DELETE') return {};
    if (contentType.indexOf('application/json') === -1 && contentType.indexOf('text/plain') === -1) return {};
...
function mapRestRequestToAction(request, url, parsedBody) {
  ...
  const adminToken = getAdminSessionTokenFromRequest(request, body);

  // Public routes
  if (parts.length === 2 && parts[1] === 'products' && method === 'GET') {
    return { payload: { action: 'get_products_public', ...query } };
  }
  if (parts.length === 3 && parts[1] === 'products' && method === 'GET') {
    return { payload: { action: 'get_product_by_slug', slug: parts[2], ...query } };
  }
  if (parts.length === 2 && parts[1] === 'product-categories' && method === 'GET') {
    return { payload: { action: 'get_product_categories', ...query } };
  }
  if (parts.length === 2 && parts[1] === 'articles' && method === 'GET') {
    return { payload: { action: 'get_articles', ...query } };
  }
  if (parts.length === 3 && parts[1] === 'articles' && method === 'GET') {
    return { payload: { action: 'get_article_by_slug', slug: parts[2], ...query } };
  }
  if (parts.length === 2 && parts[1] === 'testimonials' && method === 'GET') {
    return { payload: { action: 'get_testimonials', ...query } };
  }
  if (parts.length === 2 && parts[1] === 'banners' && method === 'GET') {
    return { payload: { action: 'get_banners', ...query } };
  }
  if (parts.length === 2 && parts[1] === 'faqs' && method === 'GET') {
    return { payload: { action: 'get_faqs', ...query } };
  }
  if (parts.length === 3 && parts[1] === 'settings' && parts[2] === 'public' && method === 'GET') {
    return { payload: { action: 'get_public_settings', ...query } };
  }
  if (parts.length === 2 && parts[1] === 'inquiries' && method === 'POST') {
    return { payload: { action: 'create_inquiry', ...body } };
  }

  // Admin routes
  if (parts.length === 4 && parts[1] === 'admin' && parts[2] === 'auth' && parts[3] === 'login' && method === 'POST') {
    return { payload: { action: 'admin_login', email: body.email || '', password: body.password || '' } };
  }
  if (parts.length === 4 && parts[1] === 'admin' && parts[2] === 'dashboard' && parts[3] === 'summary' && method === 'GET') {
    return { payload: { action: 'get_admin_dashboard_summary', auth_session_token: adminToken } };
  }

  // Admin products
  if (parts.length === 3 && parts[1] === 'admin' && parts[2] === 'products' && method === 'GET') {
    return { payload: { action: 'get_admin_products_v2', auth_session_token: adminToken, ...query } };
  }
  if (parts.length === 3 && parts[1] === 'admin' && parts[2] === 'products' && method === 'POST') {
    return { payload: { action: 'save_admin_product_v2', auth_session_token: adminToken, payload: body } };
  }
  if (parts.length === 4 && parts[1] === 'admin' && parts[2] === 'products' && method === 'PUT') {
    return { payload: { action: 'save_admin_product_v2', auth_session_token: adminToken, payload: { ...body, id: parts[3] } } };
  }
  if (parts.length === 4 && parts[1] === 'admin' && parts[2] === 'products' && method === 'DELETE') {
    return { payload: { action: 'delete_admin_product_v2', auth_session_token: adminToken, id: parts[3] } };
  }

  // Admin categories
  if (parts.length === 3 && parts[1] === 'admin' && parts[2] === 'categories' && method === 'GET') {
    return { payload: { action: 'get_admin_categories', auth_session_token: adminToken, ...query } };
  }
  if (parts.length === 3 && parts[1] === 'admin' && parts[2] === 'categories' && method === 'POST') {
    return { payload: { action: 'save_admin_category', auth_session_token: adminToken, payload: body } };
  }
  if (parts.length === 4 && parts[1] === 'admin' && parts[2] === 'categories' && method === 'PUT') {
    return { payload: { action: 'save_admin_category', auth_session_token: adminToken, payload: { ...body, id: parts[3] } } };
  }
  if (parts.length === 4 && parts[1] === 'admin' && parts[2] === 'categories' && method === 'DELETE') {
    return { payload: { action: 'delete_admin_category', auth_session_token: adminToken, id: parts[3] } };
  }

  // Admin articles
  if (parts.length === 3 && parts[1] === 'admin' && parts[2] === 'articles' && method === 'GET') {
    return { payload: { action: 'get_admin_articles_v2', auth_session_token: adminToken, ...query } };
  }
  if (parts.length === 3 && parts[1] === 'admin' && parts[2] === 'articles' && method === 'POST') {
    return { payload: { action: 'save_admin_article_v2', auth_session_token: adminToken, payload: body } };
  }
  if (parts.length === 4 && parts[1] === 'admin' && parts[2] === 'articles' && method === 'PUT') {
    return { payload: { action: 'save_admin_article_v2', auth_session_token: adminToken, payload: { ...body, id: parts[3] } } };
  }
  if (parts.length === 4 && parts[1] === 'admin' && parts[2] === 'articles' && method === 'DELETE') {
    return { payload: { action: 'delete_admin_article_v2', auth_session_token: adminToken, id: parts[3] } };
  }

  // Admin testimonials
  if (parts.length === 3 && parts[1] === 'admin' && parts[2] === 'testimonials' && method === 'GET') {
    return { payload: { action: 'get_admin_testimonials_v2', auth_session_token: adminToken, ...query } };
  }
  if (parts.length === 3 && parts[1] === 'admin' && parts[2] === 'testimonials' && method === 'POST') {
    return { payload: { action: 'save_admin_testimonial_v2', auth_session_token: adminToken, payload: body } };
  }
  if (parts.length === 4 && parts[1] === 'admin' && parts[2] === 'testimonials' && method === 'PUT') {
    return { payload: { action: 'save_admin_testimonial_v2', auth_session_token: adminToken, payload: { ...body, id: parts[3] } } };
  }
  if (parts.length === 4 && parts[1] === 'admin' && parts[2] === 'testimonials' && method === 'DELETE') {
    return { payload: { action: 'delete_admin_testimonial_v2', auth_session_token: adminToken, id: parts[3] } };
  }

  // Admin inquiries
  if (parts.length === 3 && parts[1] === 'admin' && parts[2] === 'inquiries' && method === 'GET') {
    return { payload: { action: 'get_admin_inquiries_v2', auth_session_token: adminToken, ...query } };
  }
  if (parts.length === 4 && parts[1] === 'admin' && parts[2] === 'inquiries' && method === 'PUT') {
    return { payload: { action: 'update_admin_inquiry_v2', auth_session_token: adminToken, payload: { ...body, id: parts[3] } } };
  }

  // Admin banners
  if (parts.length === 3 && parts[1] === 'admin' && parts[2] === 'banners' && method === 'GET') {
    return { payload: { action: 'get_admin_banners_v2', auth_session_token: adminToken, ...query } };
  }
  if (parts.length === 3 && parts[1] === 'admin' && parts[2] === 'banners' && method === 'POST') {
    return { payload: { action: 'save_admin_banner_v2', auth_session_token: adminToken, payload: body } };
  }
  if (parts.length === 4 && parts[1] === 'admin' && parts[2] === 'banners' && method === 'PUT') {
    return { payload: { action: 'save_admin_banner_v2', auth_session_token: adminToken, payload: { ...body, id: parts[3] } } };
  }
  if (parts.length === 4 && parts[1] === 'admin' && parts[2] === 'banners' && method === 'DELETE') {
    return { payload: { action: 'delete_admin_banner_v2', auth_session_token: adminToken, id: parts[3] } };
  }

  // Admin FAQs
  if (parts.length === 3 && parts[1] === 'admin' && parts[2] === 'faqs' && method === 'GET') {
    return { payload: { action: 'get_admin_faqs_v2', auth_session_token: adminToken, ...query } };
  }
  if (parts.length === 3 && parts[1] === 'admin' && parts[2] === 'faqs' && method === 'POST') {
    return { payload: { action: 'save_admin_faq_v2', auth_session_token: adminToken, payload: body } };
  }
  if (parts.length === 4 && parts[1] === 'admin' && parts[2] === 'faqs' && method === 'PUT') {
    return { payload: { action: 'save_admin_faq_v2', auth_session_token: adminToken, payload: { ...body, id: parts[3] } } };
  }
  if (parts.length === 4 && parts[1] === 'admin' && parts[2] === 'faqs' && method === 'DELETE') {
    return { payload: { action: 'delete_admin_faq_v2', auth_session_token: adminToken, id: parts[3] } };
  }

  // Admin settings
  if (parts.length === 3 && parts[1] === 'admin' && parts[2] === 'settings' && method === 'GET') {
    return { payload: { action: 'get_admin_data', auth_session_token: adminToken } };
  }
  if (parts.length === 3 && parts[1] === 'admin' && parts[2] === 'settings' && method === 'PUT') {
    return { payload: { action: 'update_settings', auth_session_token: adminToken, payload: body } };
  }

  return null;
}

async function handleApiRest(request, env, ctx, url) {
  if (request.method === 'OPTIONS') {
    return new Response(null, { status: 204, headers: corsHeadersFor(request, env) });
  }

  const parsedBody = await parseRequestBodyObject(request.clone());
  const mapped = mapRestRequestToAction(request, url, parsedBody);
  if (!mapped) {
    return new Response(JSON.stringify({ status: 'error', message: 'REST API route not found.' }), {
      status: 404,
      headers: { 'Content-Type': 'application/json; charset=utf-8', ...corsHeadersFor(request, env) }
    });
  }

  const headers = new Headers(request.headers);
  headers.set('Content-Type', 'application/json');
  const proxiedRequest = new Request(new URL('/api', request.url).toString(), {
    method: 'POST',
    headers,
    body: JSON.stringify(mapped.payload)
  });
  return handleApi(proxiedRequest, env, ctx);
}
...
function getApiCacheTtls(env) {
  const defaults = {
    ...
    get_public_settings: 300,
    get_products: 60,
    get_products_public: 60,
    get_product: 60,
    get_product_by_slug: 60,
    get_product_categories: 300,
    get_articles: 180,
    get_article_by_slug: 180,
    get_testimonials: 300,
    get_banners: 300,
    get_faqs: 300,
    get_page_content: 60,
    get_pages: 120
